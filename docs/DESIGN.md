...
- **Nothing decorative may swallow the drag.** Copy laid over the picture gets `pointer-events: none` — only genuinely interactive things (links, buttons) take events back. Giving every overlay child pointer-events is the bug that made the slider feel stuck: the headline and lede were eating drags they had no use for.
- **The grip clears the copy and grabs from outside itself.** It sits at 42% height on the overlay layout — below the tags, above the text — and carries an invisible 22px grab margin, so it is catchable on a phone without enlarging the visible square.
